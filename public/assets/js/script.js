...
const headerStickyWrapper = document.querySelector(
    'header'
),
    headerStickyTarget = document.querySelector(
        '.header__sticky'
    );
if (headerStickyTarget) {
    ...
    window.addEventListener('scroll', function () {
        let StickyTargetElement,
            TargetElementTopOffset = TopOffset(
                headerStickyWrapper
            ).top;
...
var swiper = new Swiper('.hero__slider--activation', {
    ...
    swiper = new Swiper(
        '.categories2__swiper--activation',
        {
            slidesPerView: 6,
            loop: !0,
            clickable: !0,
...
function offcanvsSidebar(
    openTrigger,
    closeTrigger,
    wrapper
) {
    let OpenTriggerprimary__btn = document.querySelectorAll(
        openTrigger
    ),
        closeTriggerprimary__btn = document.querySelector(
            closeTrigger
        ),
        ...
        eventTarget.closest(wrapper) ||
            ...
            (WrapperSidebar.classList.remove('active'),
                document
                    .querySelector('body')
                    .classList.remove(
                        `${wrapperOverlay}_active`
                    ));
    }
    OpenTriggerprimary__btn &&
        WrapperSidebar &&
        OpenTriggerprimary__btn.forEach(function (
            singleItem
        ) {
        ...
        WrapperSidebar &&
        closeTriggerprimary__btn.addEventListener(
            'click',
            function (e) {
                null != e.target.dataset.offcanvas &&
                    (WrapperSidebar.classList.remove(
                        'active'
                    ),
                        document
                            .querySelector('body')
...
quantityWrapper &&
    quantityWrapper.forEach(function (singleItem) {
        let increaseButton = singleItem.querySelector(
            '.increase'
        ),
            decreaseButton = singleItem.querySelector(
                '.decrease'
            );
        increaseButton.addEventListener('click', function (
            ...
            let input =
                e.target.previousElementSibling.children[0];
...
function customAccordion(
    ...
    document
        .querySelectorAll(accordionWrapper)
        .forEach(function (item) {
            item.addEventListener('click', function (evt) {
                let itemTarget = evt.target;
                if (
                    itemTarget.classList.contains(
                    ...
                    let singleAccordionWrapper = itemTarget.closest(
                        singleItem
                    ),
                        singleAccordionBody = singleAccordionWrapper.querySelector(
                            accordionBody
                        );
                    singleAccordionWrapper.classList.contains(
                        'active'
                    )
                        ? (singleAccordionWrapper.classList.remove(
                            'active'
                        ),
                            slideUp(singleAccordionBody))
                        : (singleAccordionWrapper.classList.add(
                            'active'
                        ),
                            slideDown(singleAccordionBody),
                            getSiblings(
                                singleAccordionWrapper
                            ).forEach(function (item) {
                                let sibllingSingleAccordionBody = item.querySelector(
                                    accordionBody
                                );
    ...
    (accordion = !1),
        document
            ...
            .addEventListener('click', function (evt) {
                ...
                if (
                    singleItemTarget.classList.contains(
                        'footer__widget--button'
                    )
                ) {
                    const footerWidget = singleItemTarget.closest(
                        '.footer__widget'
                    ),
                        footerWidgetInner = footerWidget.querySelector(
                            '.footer__widget--inner'
                        );
                    footerWidget.classList.contains(
                        'active'
                    )
                        ? (footerWidget.classList.remove(
                            'active'
                        ),
                            slideUp(footerWidgetInner))
                        : (footerWidget.classList.add(
                            'active'
                        ),
...
window.addEventListener('load', function () {
    ...
    window.addEventListener('resize', function () {
        document
            .querySelectorAll('.footer__widget')
            .forEach(function (item) {
...
if (wrapper) {
    const counters = wrapper.querySelectorAll(
        '.js-counter'
    ),
        duration = 1e3;
    let isCounted = !1;
    document.addEventListener('scroll', function () {
        const wrapperPos =
            wrapper.offsetTop - window.innerHeight;
        !isCounted &&
...
const offcanvasHeader = function () {
    const offcanvasOpen = document.querySelector(
        ...
        offcanvasClose = document.querySelector(
            '.offcanvas__close--btn'
        ),
        offcanvasHeader = document.querySelector(
            '.offcanvas__header'
        ),
        offcanvasMenu = document.querySelector(
            '.offcanvas__menu'
        ),
    ...
    offcanvasMenu &&
        offcanvasMenu
            ...
            .forEach(function (ul) {
                const subMenuToggle = document.createElement(
                    'button'
                );
        ...
        offcanvasOpen &&
        offcanvasOpen.addEventListener(
            'click',
            function (e) {
                e.preventDefault(),
                    null !=
                    e.target.dataset.offcanvas &&
        ...
        offcanvasClose &&
        offcanvasClose.addEventListener(
            'click',
            function (e) {
                e.preventDefault(),
                    null !=
                    e.target.dataset.offcanvas &&
                    (offcanvasHeader.classList.remove(
                        'open'
                    ),
    ...
    mobileMenuWrapper &&
        mobileMenuWrapper.addEventListener(
            'click',
            function (e) {
                        ...
                        ? (targetElement.classList.remove(
                            ...
                            parent
                                ...
                                .forEach(function (subMenu) {
                                    subMenu.parentElement.classList.remove(
                                        'active'
                                    ),
                                        subMenu.nextElementSibling.classList.remove(
                                            'active'
                                        ),
                                        slideUp(subMenu);
                                }))
...
const categoryMobileMenu = function () {
    ...
    CategorySubMenu &&
        CategorySubMenu.querySelectorAll(
            '.category__sub--menu'
        ).forEach(function (ul) {
            let catsubMenuToggle = document.createElement(
                'button'
            );
            catsubMenuToggle.classList.add(
    ...
    categoryMenuWrapper &&
        categoryMenuWrapper.addEventListener(
            'click',
            function (e) {
                let targetElement = e.target;
                if (
...
const newsletterPopup = function () {
    let newsletterWrapper = document.querySelector(
        '.newsletter__popup'
    ),
        newsletterCloseButton = document.querySelector(
            '.newsletter__popup--close__btn'
        ),
        dontShowPopup = document.querySelector(
            '#newsletter__dont--show'
        ),
        popuDontShowMode = localStorage.getItem(
            'newsletter__show'
        );
    newsletterWrapper &&
        ...
        window.addEventListener('load', (event) => {
            setTimeout(function () {
                document.body.classList.add(
                    ...
                    newsletterWrapper.classList.add(
                        'newsletter__show'
                    ),
                        ...
                        function (event) {
                            event.target.closest(
                                ...
                                (document.body.classList.remove(
                                    ...
                                    newsletterWrapper.classList.remove(
                                        'newsletter__show'
                                    ));
                        }
                    ),
                    ...
                    dontShowPopup.addEventListener(
                        'click',
                        function () {
